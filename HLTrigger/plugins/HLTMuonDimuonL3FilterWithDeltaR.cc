...
#include "HLTMuonDimuonL3FilterWithDeltaR.h"
...
HLTMuonDimuonL3FilterWithDeltaR::HLTMuonDimuonL3FilterWithDeltaR(const edm::ParameterSet& iConfig)
    : HLTFilter(iConfig),
      ...
      min_DeltaRMuMu_(iConfig.getParameter<double>("MinDeltaRMuMu")),
      max_DeltaRMuMu_(iConfig.getParameter<double>("MaxDeltaRMuMu")),
  ...
  LogDebug("HLTMuonDimuonL3FilterWithDeltaR") << " CandTag/MinN/MaxEta/MinNhits/MaxDr/MaxDz/MinPt1/MinPt2/MinInvMass/MaxInvMass/"
...
HLTMuonDimuonL3FilterWithDeltaR::~HLTMuonDimuonL3FilterWithDeltaR() = default;

void HLTMuonDimuonL3FilterWithDeltaR::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<double>("MinDeltaRMuMu", -1.0);
  desc.add<double>("MaxDeltaRMuMu", 99999.9);
...
bool HLTMuonDimuonL3FilterWithDeltaR::hltFilter(edm::Event& iEvent,
  ...
  for (; L2toL3s_it1 != L2toL3s_end; ++L2toL3s_it1) {
    ...
    for (; iTk1 != maxItk1; iTk1++) {
      ...
      LogDebug("HLTMuonDimuonL3FilterWithDeltaR") << " 1st muon in loop: q*pt= " << tk1->charge() * tk1->pt() << " ("
      ...
      LogDebug("HLTMuonDimuonL3FilterWithDeltaR") << " ... 1st muon in loop, pt1= " << cand1->pt();
      ...
      for (; L2toL3s_it2 != L2toL3s_end; ++L2toL3s_it2) {
        ...
        for (; iTk2 != maxItk2; iTk2++) {
          ...
          LogDebug("HLTMuonDimuonL3FilterWithDeltaR") << " 2nd muon in loop: q*pt= " << tk2->charge() * tk2->pt() << " ("
          ...
          // Pt threshold cut
          // Don't convert to 90% efficiency threshold
          LogDebug("HLTMuonDimuonL3FilterWithDeltaR") << " ... 2nd muon in loop, pt2= " << cand2->pt();
          ...
          // Add this pair
          n++;
          LogDebug("HLTMuonDimuonL3FilterWithDeltaR")
  ...
  for (; MuonToL3s_it1 != MuonToL3s_end; ++MuonToL3s_it1) {
    ...
    for (; MuonToL3s_it2 != MuonToL3s_end; ++MuonToL3s_it2) {
      ...
      n++;
      LogDebug("HLTMuonDimuonL3FilterWithDeltaR") << " L3FromL1 Track1 passing filter: pt= " << cand1->pt()
                                        << ", eta: " << cand1->eta();
      LogDebug("HLTMuonDimuonL3FilterWithDeltaR") << " L3FromL1 Track2 passing filter: pt= " << cand2->pt()
                                        << ", eta: " << cand2->eta();
  ...
  LogDebug("HLTMuonDimuonL3FilterWithDeltaR") << " >>>>> Result of HLTMuonDimuonL3FilterWithDeltaR is " << accept
                                    << ", number of muon pairs passing thresholds= " << n;

  return accept;
}

bool HLTMuonDimuonL3FilterWithDeltaR::triggeredByLevel2(TrackRef const& staTrack, vector<RecoChargedCandidateRef> const& vcands) {
  bool ok = false;
...
bool HLTMuonDimuonL3FilterWithDeltaR::applyMuonSelection(const RecoChargedCandidateRef& cand, const BeamSpot& beamSpot) const {
  // eta cut
...
bool HLTMuonDimuonL3FilterWithDeltaR::applyDiMuonSelection(const RecoChargedCandidateRef& cand1,
  ...
  if (acop > M_PI)
    acop = 2 * M_PI - acop;
  acop = M_PI - acop;
  LogDebug("HLTMuonDimuonL3FilterWithDeltaR") << " ... 1-2 acop= " << acop;
  if (acop < min_Acop_)
    return false;
  if (acop > max_Acop_)
    return false;
  ...
  // deltaR between the Muons
  double deltaRmumu = deltaR2( cand1->eta(), cand1->phi(), cand2->eta(), cand2->phi() );
  if (deltaRmumu < min_DeltaRMuMu_)
    return false;
  if (deltaRmumu > max_DeltaRMuMu_)
    return false;
  ...
  double pt12 = p.pt();
  LogDebug("HLTMuonDimuonL3FilterWithDeltaR") << " ... 1-2 pt12= " << pt12;

  double ptLx1 = cand1->pt();
  double ptLx2 = cand2->pt();
  double invmass = abs(p.mass());
  // if (invmass>0) invmass = sqrt(invmass); else invmass = 0;
  LogDebug("HLTMuonDimuonL3FilterWithDeltaR") << " ... 1-2 invmass= " << invmass;
  bool proceed = false;
...
DEFINE_FWK_MODULE(HLTMuonDimuonL3FilterWithDeltaR);
