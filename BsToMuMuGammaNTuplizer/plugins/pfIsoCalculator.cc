...
double pfIsoCalculator::getPfIso(const reco::SuperCluster& sc, int pfId,
                                 double r1, double r2, double threshold)
{
  double scEta = sc.eta();
  double scPhi = sc.phi();
  ...
  //totalEt -= (sc.rawEnergy() / std::cosh(sc.eta()));
  ...
  for (auto pf = candidatesView->begin(); pf != candidatesView->end(); ++pf) {
    ...
    double dEta = std::abs(scEta - pfEta);
    double dPhi = reco::deltaPhi(pfPhi, scPhi);
    double dR2 = dEta*dEta + dPhi*dPhi;
    double pfPt = pf->pt();
